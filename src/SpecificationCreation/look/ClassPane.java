...
public class ClassPane implements ActionListener
{
	...
	{	//create example 'anchor' with class name
		...
		root = new ClassTreeNode(getNL(c, className, true), shaded, false, false);
		root.setRoot(true);
		createMenu(c, root);
		...
		for (OntProperty prop : properties)
		{
			...
			{
				ClassTreeNode sub = new ClassTreeNode(submenu, false, false, true);
	...
	{	//find if there is a specification for this property
		...
		ClassTreeNode result = new ClassTreeNode(nl, shaded, specExists, false);
 		...
 		if (e.getSource() == backBtn)
   	   	...
   	   	else if (e.getSource() == submenuBtn)
   	   	{
   	   	    ...
   	   	    if ((submenu != null) && (submenu.length() > 0))
   	   	    {
   	   	    	...
   	   	    	{
   	   	    		ClassTreeNode node = new ClassTreeNode(submenu, false, false, true);
